...
using Application.Queries.GetBookingsByUserHandler;
...
// DI Handler
builder.Services.AddScoped<GetAllBookingHandler>();
builder.Services.AddScoped<GetBookingByIdHandler>();
builder.Services.AddScoped<GetBookingsByUserHandler>();
...
{
    cfg.RegisterServicesFromAssembly(typeof(GetAllBookingHandler).Assembly);
});

builder.Services.AddMediatR(cfg => 
{
    cfg.RegisterServicesFromAssembly(typeof(GetBookingByIdHandler).Assembly);
});

builder.Services.AddMediatR(cfg => 
{
    cfg.RegisterServicesFromAssembly(typeof(GetBookingsByUserHandler).Assembly);
});
...
builder.Services.AddMediatR(cfg => 
{
    cfg.RegisterServicesFromAssembly(typeof(CheckInBookingHandler).Assembly);
});
...
{
    client.AddPolicy("AllowRoomService", policy =>
    {
        // mở cho tất cả 
            // policy.AllowAnyOrigin()
            //     .AllowAnyHeader()
            //     .AllowAnyMethod();
        policy.WithOrigins("https://localhost:7138") // Địa chỉ của RoomService
              .AllowAnyHeader()
              .AllowAnyMethod();
    });
});
