...
// DI Handler
builder.Services.AddScoped<GetAllCustomerHandler>();
...
builder.Services.AddScoped<RemoveCustomerHandler>();

// MediatR (CQRS)
builder.Services.AddMediatR(cfg => 
{
    cfg.RegisterServicesFromAssembly(typeof(GetAllCustomerHandler).Assembly);
});
...
builder.Services.AddMediatR(cfg => 
{
    cfg.RegisterServicesFromAssembly(typeof(UpdateCustomerHandler).Assembly);
});

builder.Services.AddMediatR(cfg => 
{
    cfg.RegisterServicesFromAssembly(typeof(RemoveCustomerHandler).Assembly);
});

//DI AutoMapper
builder.Services.AddAutoMapper(cf=>{}, typeof(CustomerProfile));
...
// dotnet ef dbcontext scaffold "Name=DBConnectionstring" Microsoft.EntityFrameworkCore.SqlServer --context CustomerDbContext --context-dir src/Infrastructure/Data --output-dir src/Domain/Entities --table Customers --data-annotations --force